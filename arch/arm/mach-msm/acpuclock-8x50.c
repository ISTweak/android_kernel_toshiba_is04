...
struct clkctl_acpu_speed acpu_freq_tbl_998[] = {
	...
	{ 1,  128000, ACPU_PLL_1, 1, 5, 0, 0,  14000, 2,    0,  900},
	{ 1,  235930, ACPU_PLL_0, 4, 0, 0, 0,  29000, 0,    0,  930},
	...
	{ 0,  729600, ACPU_PLL_3, 0, 0, 0, 0, 117000, 1, 0x13, 1075},
	...
	{ 0,  921600, ACPU_PLL_3, 0, 0, 0, 0, 128000, 1, 0x18, 1175},
	{ 0,  960000, ACPU_PLL_3, 0, 0, 0, 0, 128000, 1, 0x19, 1175},
	{ 1,  998400, ACPU_PLL_3, 0, 0, 0, 0, 128000, 1, 0x1A, 1260},
	{ 1, 1113600, ACPU_PLL_3, 0, 0, 0, 0, 128000, 1, 0x1B, 1300},
	{ 1, 1228800, ACPU_PLL_3, 0, 0, 0, 0, 128000, 1, 0x1C, 1300},
	{ 1, 1267200, ACPU_PLL_3, 0, 0, 0, 0, 192000, 1, 0x1D, 1340},
	{ 1, 1305600, ACPU_PLL_3, 0, 0, 0, 0, 256000, 1, 0x1E, 1380},
	{ 0, 1312000, ACPU_PLL_3, 0, 0, 0, 0, 256000, 1, 0x1F, 1380},
	{ 0, 1318400, ACPU_PLL_3, 0, 0, 0, 0, 256000, 1, 0x20, 1380},
	{ 0, 1324800, ACPU_PLL_3, 0, 0, 0, 0, 256000, 1, 0x21, 1380},
...
void acpuclk_set_vdd(unsigned acpu_khz, int vdd)
{
	...
	for (i = 0; acpu_freq_tbl_998[i].acpuclk_khz; i++) {
		if (acpu_freq_tbl_998[i].use_for_scaling) {
			if (acpu_khz == 0)
				acpu_freq_tbl_998[i].vdd = min(max((acpu_freq_tbl_998[i].vdd + vdd), MAX8649_MIN_DCDC2), MAX8649_MAX_DCDC2);
			else if (acpu_freq_tbl_998[i].acpuclk_khz == acpu_khz)
				acpu_freq_tbl_998[i].vdd = min(max(vdd, MAX8649_MIN_DCDC2), MAX8649_MAX_DCDC2);
		}
	}
	mutex_unlock(&drv_state.lock);
}
...
static void __init acpu_freq_tbl_fixup(void)
{
	...
	case 0x30:
	case 0x00:
		max_acpu_khz = 1420800; //998400;
		break;
	case 0x10:
		max_acpu_khz = 1420800;
		break;
	default:
