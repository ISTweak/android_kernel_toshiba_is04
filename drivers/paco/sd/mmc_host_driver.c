...
int mmc_host_io_ctrl(int cmd)
{
    ...
    case SD_RST_ON:
        XYZ("SD_RST_ON is '%c' +", gpio_get_value(154) ? 'H' : 'L');
    ...
    case SD_VDD_ON:
        XYZ("SD_VDD_ON is '%c' +", gpio_get_value(31) ? 'H' : 'L');
        gpio_set_value(31, 1);
        XYZ("SD_VDD_ON is '%c' +", gpio_get_value(31) ? 'H' : 'L');
        break;

    case SD_VDD_OFF:
        XYZ("SD_VDD_OFF is '%c' +", gpio_get_value(31) ? 'H' : 'L');
        gpio_set_value(31, 0);
        XYZ("SD_VDD_OFF is '%c' +", gpio_get_value(31) ? 'H' : 'L');
        break;

    case SD_CLK_ON:
        XYZ("SD_CLK_ON is '%c' +", gpio_get_value(104) ? 'H' : 'L');
        gpio_set_value(104, 1);
        XYZ("SD_CLK_ON is '%c' +", gpio_get_value(104) ? 'H' : 'L');
        break;

    case SD_CLK_OFF:
        XYZ("SD_CLK_OFF is '%c' +", gpio_get_value(104) ? 'H' : 'L');
